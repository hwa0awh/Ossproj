...
## Git 브랜치 활용
- 메인 브랜치에서 프로젝트 구조 및 기본 파일 추가
- load_subjects 브랜치에서 과목 정보 불러오는 기능 추가
- 메인 브랜치에 load_subjects 브랜치 병합
- No_overlap 브랜치에서 충돌 없는 시간표 조합 후 출력 기능 추가
- 메인 브랜치에 No_overlap 브랜치 병합
- Option_select 브랜치에서 조건에 맞는 시간표 추천 기능 추가
...
- 별도의 외부 라이브러리 설치는 **필요하지 않습니다**.  
  (`ossproject.py`는 표준 라이브러리만 사용합니다.)

### 2. 실행 방법

1. 저장소를 클론하거나 ZIP 파일로 다운로드하여 압축을 풉니다.  
2. `ossproject.py`와 `subjects.txt`가 **같은 폴더에 있어야 합니다**.  
3. vscode를 열고, 2번의 폴더를 엽니다.  
4. subjects.txt 파일에 시간표를 형식에 맞게 적고 저장합니다.
5. 실행합니다.  

📌 주의사항
subjects.txt 파일 형식이 올바르지 않으면 해당 줄은 자동으로 무시됩니다.

예: 요일 및 시간 정보가 부족하거나 형식이 잘못된 경우 다음 메시지가 출력됩니다.
[무시됨] 잘못된 형식: (잘못된 줄 내용)
...
---------------------------------------------------------------------------  
 ...
 10.5~12.0   객체지향(2)       -----     이산수학(1)       -----       -----  
 13.5~15.0   데이터구조(1)    머신러닝(1)       -----       -----       -----  
 15.0~16.5   확률과통계(1)      -----       -----     OSS개발(2)      -----  
 16.5~18.0   객체지향(2)     OSS개발(2)      -----     확률과통계(1)      -----  
